...
export function MemberList({ serverId }: { serverId: Id<"servers"> }) {
  ...
  if (!members || !server) {
    return (
      <aside className="hidden w-60 shrink-0 border-l border-border/40 bg-sidebar/40 lg:block" />
    )
  }
          ...
          return (
            ...
            >
              <div className="relative">
                ...
                <span
                  className={`absolute -right-0.5 -bottom-0.5 size-2 rounded-full ring-2 ring-sidebar ${STATUS_COLORS[status]}`}
                />
              </div>
              ...
              {canManage ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button
                      type="button"
                      className="hidden size-6 place-items-center rounded text-muted-foreground hover:bg-muted hover:text-foreground group-hover:grid"
